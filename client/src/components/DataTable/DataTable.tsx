...
const DataTable : React.FunctionComponent<IDataTableProps> = (props) => {
    ...
    const renderTableData  = () => {
        ...
        return data.map((item) => {
            return <tr key={item._id} className=' border-b text-white bg-gray-700 hover:bg-white hover:text-gray-800 '>
                <td className=' px-6 py-3 break-words'>
                    <Link to={`${serverUrl}/shortUrl/${item.fullUrl}`} target='_blank' rel='noreferrer noopener'>{item.fullUrl}</Link>
                </td>
